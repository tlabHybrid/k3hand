...
class k3:
    ...
    def lock_servo(self, id):
        if self.h2s.send(Header.WRITE, Address.SYS_ULK, 1, id, Command.LOCK):
    ...
    def get_angle(self, id):
        if self.h2s.send(Header.READ, Address.M_POS, 2, id):
            i = DataProcessor._decode_int16(self.h2s.rtn)
    ...
    def get_speed(self, id):
        if self.h2s.send(Header.READ, Address.FB_SC, 1, id):
            self.speeds[id] = DataProcessor._int2speed(self.h2s.rxCmd[1])
            return self.speeds[id]
    
    def send_speed(self, id, speed):
        if self.h2s.send(Header.WRITE, Address.FB_SC, 1, id, DataProcessor._speed2int(speed)):
            self.speeds[id] = speed

    def get_temp(self, id):
        if self.h2s.send(Header.READ, Address.M_TEMP, 2, id):
            return DataProcessor._decode_int16(self.h2s.rxCmd[1:3])
    
    def get_ang_min(self, id):
        if self.h2s.send(Header.READ, Address.FB_POSL, 2, id):
            return DataProcessor._int2angle(DataProcessor._decode_int16(self.h2s.rxCmd[1:3]))
    
    def get_ang_max(self, id):
        if self.h2s.send(Header.READ, Address.FB_POSH, 2, id):
            return DataProcessor._int2angle(DataProcessor._decode_int16(self.h2s.rxCmd[1:3]))

    def send_ang_max(self, id, angle):
        if self.h2s.send(Header.WRITE, Address.FB_POSH, 2, id, DataProcessor._angle2int(angle)):
    ...
    def send_ang_mins(self, angles, id_list=list(range(8))):
        if self.h2s.send(Header.WRITE, Address.FB_POSL, 2, id_list, list(map(DataProcessor._angle2int, angles))):
            for i in range(len(id_list)):
                self.ang_min[id_list[i]] = angles[i]

    def send_ang_maxs(self, angles, id_list=list(range(8))):
        if self.h2s.send(Header.WRITE, Address.FB_POSH, 2, id_list, list(map(DataProcessor._angle2int, angles))):
            for i in range(len(id_list)):
                self.ang_max[id_list[i]] = angles[i]

    def get_servo_stat(self, id):
        if self.h2s.send(Header.READ, Address.SYS_ULK, 1, id):
            if self.h2s.rxCmd[1] == Command.LOCK:
                self.servo_lock[id] = True
            else:
                self.servo_lock[id] = False
